...
def transition_model(corpus, page, damping_factor):
    ...
    if len(corpus[page]) == 0:
        ...
        for key in corpus:
            dictionary[key] = (1 - damping_factor) /len(corpus)
        sum_other = 0
        for key in corpus[page]:
            if key != page:
                dictionary[key] += damping_factor/len(corpus[page])
                sum_other += damping_factor/len(corpus[page])
        dictionary[page] += 1 - sum_other
    return dictionary
