...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        for word_x in self.domains[x]:
            # Vérifie s'il existe au moins un mot dans le domaine de y
            # tel que la lettre à la position j corresponde à la lettre i de word_x
    ...
    def consistent(self, assignment):
        ...
        """
        # Tous les mots doivent être différents
        if len(set(assignment.values())) < len(assignment):
            return False

        # Vérifier que chaque mot correspond à la longueur de la variable
        for var, word in assignment.items():
            if len(word) != var.length:
                return False

        # Vérifier les contraintes de recoupement
